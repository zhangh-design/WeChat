...
好像不太对是不是，这个图片只显示了一半对不对，它不是被压缩了啊很明显的是被截断了，然后高度是对的我们刚才说了高度不影响多高都可以它可以撑开，那这个问题是什么呢，我们看下`iphone6`这个模拟器上面宽度只有`375`（这里要注意单位是pt，我这里先不写下面会讲解），它只有`375`（这里要注意单位是pt，我这里先不写下面会讲解）那怎么去显示`750px`呢，这个原因是什么呢？
